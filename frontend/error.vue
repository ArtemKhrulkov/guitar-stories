  ...
  <v-app>
    <v-container class="fill-height justify-center">
      <v-card class="text-center pa-8" max-width="600">
        <IconifyIcon :icon="errorIcon" size="80" :color="errorColor" class="mb-4" />
        ...
        <div class="d-flex justify-center gap-4">
          <v-btn color="primary" size="large" to="/">
            <IconifyIcon icon="mdi-home" class="mr-2" />
            Go Home
          </v-btn>
...
const errorMessage = computed(() => {
  ...
  return (
    messages[props.statusCode || 500] || props.error?.message || 'An unexpected error occurred.'
  );
});
