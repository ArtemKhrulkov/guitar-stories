<template>
  <v-container fluid class="pa-6">
    ...
    <v-card v-else-if="error" color="error" class="pa-6 text-center" role="alert">
      ...
      <v-btn to="/guitars" color="white" variant="flat"> Back to Catalog </v-btn>
    </v-card>
    ...
    <template v-else-if="guitar">
      ...
      <v-breadcrumbs :items="breadcrumbs" class="px-0 mb-4" aria-label="Breadcrumb navigation">
        <template #divider>
          <IconifyIcon icon="mdi-chevron-right" class="align-middle" aria-hidden="true" />
        </template>
      </v-breadcrumbs>
        ...
        <v-col cols="12" md="7">
          ...
          <v-card color="surface" class="pa-4 mb-6">
            <div class="d-flex align-center">
              ...
              />
              <div>
                <div class="text-caption text-medium-emphasis">Price Range</div>
            ...
            <v-expansion-panel title="Specifications">
              <v-expansion-panel-text>
                <v-list density="compact" aria-label="Guitar specifications">
                  ...
                  >
                    <template #prepend>
                      <IconifyIcon icon="mdi-cog" aria-hidden="true" class="mr-2" />
                    </template>
                  </v-list-item>
                </v-list>
              </v-expansion-panel-text>
          ...
          <v-card v-if="guitar.history" class="mb-6">
            <v-card-title class="d-flex align-center">
              <IconifyIcon icon="mdi-history" color="primary" class="mr-2" aria-hidden="true" />
              History
            </v-card-title>
          ...
          <!-- Famous Players -->
          <v-card v-if="guitar.players && guitar.players.length > 0" class="mb-6">
            <v-card-title class="d-flex align-center">
              <IconifyIcon
            ...
            <v-card-text>
              <v-chip-group aria-label="Famous players who use this guitar">
                <v-chip
                  ...
                  class="ma-1"
                >
                  <v-avatar start size="24" aria-hidden="true">
                    <NuxtImg
                      :src="player.image_url || 'https://via.placeholder.com/24'"
                      :alt="player.name"
                      width="24"
                      height="24"
                      loading="lazy"
          ...
          <!-- Purchase Links -->
          <PurchaseLinks v-if="guitar.purchase_links" :links="guitar.purchase_links" />
        </v-col>
      </v-row>
    </template>
  </v-container>
