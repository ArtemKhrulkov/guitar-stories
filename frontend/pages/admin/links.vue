    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
      <v-card class="pa-6 mb-6">
        ...
        <v-form @submit.prevent="handleAddLink">
          ...
          <v-switch v-model="newLink.in_stock" label="In Stock" color="success" class="mb-4" />

          <v-btn type="submit" color="primary" :loading="isAdding" :disabled="!canAddLink">
            Add Link
          </v-btn>
        </v-form>
      </v-card>
      ...
      <v-card class="pa-6">
        ...
        <div v-else-if="guitarsWithLinks.length === 0" class="text-center py-8 text-gray-400">
          No links found. Search for a guitar above to add links.
        </div>

        <div v-else>
          <div v-for="guitar in guitarsWithLinks" :key="guitar.id" class="mb-6">
            <div class="flex items-center gap-2 mb-2">
              <span class="font-semibold">{{ guitar.brand?.name }}</span>
              <span class="text-gray-400">{{ guitar.model }}</span>
            </div>
            ...
            <v-table density="compact" class="bg-gray-800">
              ...
              <tbody>
                <tr v-for="link in guitar.links" :key="link.id">
                  ...
                  <td>
                    <span v-if="link.price_rub">{{ formatPrice(link.price_rub) }} RUB</span>
                    <span v-if="link.price_usd" class="ml-2 text-gray-400"
                      >({{ formatPrice(link.price_usd) }} USD)</span
                    >
                  </td>
                  <td>
                    <v-icon :color="link.in_stock ? 'success' : 'error'" size="small">
                      {{ link.in_stock ? 'mdi:check-circle' : 'mdi:close-circle' }}
                    </v-icon>
                  </td>
                  <td>
                    <v-btn
...
import type { Guitar, PurchaseLink, LinkInput } from '~/composables/useAdminLinks';
  ...
  try {
    ...
    for (const guitar of response.guitars) {
      try {
        ...
        console.error('Failed to fetch guitar detail', e.message);
