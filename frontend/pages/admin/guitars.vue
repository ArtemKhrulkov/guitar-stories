<template>
  <div class="min-h-screen bg-gray-900">
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
      ...
      <div v-else-if="guitars.length === 0" class="text-center py-12 text-gray-400">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="relative">
            <v-img
              :src="guitar.image_url || 'https://via.placeholder.com/400x300?text=No+Image'"
              height="200"
              cover
              class="bg-gray-700"
            />
          ...
          <v-card-title> {{ guitar.brand?.name }} {{ guitar.model }} </v-card-title>
          ...
          <v-expand-transition>
            <div v-if="expandedId === guitar.id" class="px-4 pb-4">
              ...
              <div class="flex items-center justify-between mb-2">
                <span class="font-semibold">Purchase Links</span>
              ...
              <div v-if="guitarLinks[guitar.id]?.length === 0" class="text-gray-400 text-sm">
                No links yet
              </div>
              ...
              <div v-else class="space-y-2">
                ...
                >
                  <div class="flex items-center gap-2">
                    <v-chip size="x-small" :color="getPlatformColor(link.platform)">
                      {{ link.platform }}
                    </v-chip>
                    <a
                      :href="link.url"
    ...
    <v-dialog v-model="createDialog.show" max-width="600">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="createDialog.loading" @click="handleCreateGuitar">
            Create
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="editDialog.show" max-width="600">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" :loading="editDialog.loading" @click="handleUpdateGuitar">
            Save
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="addLinkDialog.show" max-width="500">
      <v-card>
        <v-card-title>Add Link to {{ addLinkDialog.guitar?.model }}</v-card-title>
        <v-card-text>
          <v-select
          ...
          <v-switch v-model="addLinkDialog.data.in_stock" label="In Stock" color="success" />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn @click="addLinkDialog.show = false">Cancel</v-btn>
          <v-btn color="primary" :loading="addLinkDialog.loading" @click="handleAddLink">
            Add
          </v-btn>
        </v-card-actions>
      </v-card>
...
const handleCreateGuitar = async () => {
  ...
  try {
    let specs: Specifications | undefined;
    ...
    const data: GuitarCreate = {
      ...
      guitar_type: createDialog.value.data.guitar_type as 'electric' | 'acoustic' | 'bass',
...
const handleUpdateGuitar = async () => {
  ...
  try {
    ...
    if (editDialog.value.data.image_url !== (editDialog.value.guitar?.image_url || '')) {
      updateData.image_url = editDialog.value.data.image_url;
    }
    if (editDialog.value.data.guitar_type !== editDialog.value.guitar?.guitar_type) {
      updateData.guitar_type = editDialog.value.data.guitar_type as
        | 'electric'
        | 'acoustic'
        | 'bass';
    }
    if (editDialog.value.data.price_range !== (editDialog.value.guitar?.price_range || '')) {
      updateData.price_range = editDialog.value.data.price_range;
    }
    if (editDialog.value.data.history !== (editDialog.value.guitar?.history || '')) {
...
const handleDeleteLink = async (linkId: string) => {
  try {
    ...
    for (const guitarId in guitarLinks.value) {
      guitarLinks.value[guitarId] = guitarLinks.value[guitarId].filter((l) => l.id !== linkId);
    }
  } catch (err: unknown) {
    const e = err as { data?: { error?: string } };
    snackbar.value = {
