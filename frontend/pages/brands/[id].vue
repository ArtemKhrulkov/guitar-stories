<template>
  <v-container fluid class="pa-6">
    ...
    <v-card v-else-if="error" color="error" class="pa-6 text-center" role="alert">
      <IconifyIcon icon="mdi-alert-circle" size="48" class="mb-4" aria-hidden="true"></IconifyIcon>
      <h2 class="text-h5 mb-2">Error Loading Brand</h2>
      <p class="mb-4">{{ error }}</p>
      <v-btn to="/brands" color="white" variant="flat">
        Back to Brands
      </v-btn>
    </v-card>
    ...
    <template v-else-if="brand">
      ...
      <v-breadcrumbs :items="breadcrumbs" class="px-0 mb-4" aria-label="Breadcrumb navigation">
        <template v-slot:divider>
          <IconifyIcon icon="mdi-chevron-right" aria-hidden="true"></IconifyIcon>
        </template>
      </v-breadcrumbs>
      ...
      <v-row class="mt-8">
        <v-col cols="12">
          <h2 class="text-h5 font-weight-bold mb-6">
            <IconifyIcon icon="mdi-guitar-electric" color="primary" class="mr-2" aria-hidden="true"></IconifyIcon>
      ...
      <v-row v-if="guitars.length === 0">
        <v-col cols="12">
          <v-card class="text-center pa-12">
            <IconifyIcon icon="mdi-guitar-electric" size="64" color="grey" aria-hidden="true"></IconifyIcon>
